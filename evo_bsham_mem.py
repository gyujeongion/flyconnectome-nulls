"""MEMORY VARIANT of evo_bsham.py (research log 57). Identical operations, random draws and order; the only
change is where evaluation is forced (lines marked # MEM): inside mutate() after each full-matrix step, and around
reproduction, so that several full copies of the population's weight matrices are not alive at once (peak 33 GB at
population 4,608 on a 32 GB machine). Bit-identity with evo_bsham.py was checked before use (log 57).
"""
"""evo_bsham.py: evo_dose.py + boundary-targeted, shortcut-free shams BS<pct> (PROTOCOL_BSHAM.md).

Changes from evo_dose.py (hash dc27fa100d16d8a7), and nothing else:
  1. BS<pct>: for each seed, as many target rotations as that seed's AS<pct> took. Each rotation takes the same two
     boundary edges an AS swap takes (an olfactory-receptor edge ORN->X, X not descending, and a descending-input
     edge Y->DN, Y not sensory) plus one interior edge Z->W (Z not sensory, W neither sensory nor descending), and
     rotates their targets: ORN->W, Y->X, Z->DN. Against the AS swap (ORN->DN, Y->X) the only difference is that the
     olfactory weight lands on an interior group and the descending group receives Z's weight instead: no shortcut.
     Degrees and signs are kept; Z->W is chosen, among up to 64 valid candidates, with |w| closest to |w(ORN->X)|.
  2. Elites: the top 16 per island by the current generation's fitness are saved BEFORE the population is replaced
     (evo_dose.py saved 8 indices of the pre-selection order from the already-replaced population, i.e. samples).
  3. After the last generation the final elites are evaluated on four fresh world batches (seeds 10001-10004, 8
     lifetimes each), intact and with olfaction blocked, written to fresh.jsonl. The common-garden probe is unchanged.
evo_dose.py: evo_swap.py + graded shortcut levels AS<pct> and a rewiring-matched sham control SHAM (PROTOCOL_DOSE.md).
...
ORN_IDX = np.array(sorted(set(int(i) for i in B["in_orn"])))
def smell_shortcut_share(Wg):
    dnm = np.zeros(K, bool); dnm[np.asarray(dn, int)] = True
    return float(np.abs(Wg[np.ix_(dnm, ORN_IDX)]).sum() / (np.abs(Wg[:, ORN_IDX]).sum() + 1e-12))

def transplant_shortcuts(Wg, nrng, target, add):
    """Degree-preserving target swaps between an olfactory-receptor edge and a non-sensory edge.
    add=True : ORN->X (X not descending) + Y->DN  ->  ORN->DN + Y->X   (creates smell->motor shortcuts)
    add=False: ORN->DN + Y->X (X not descending, not sensory)  ->  ORN->X + Y->DN   (removes them)
    Every group keeps its in- and out-degree; each weight stays with its source (Dale sign kept)."""
    W = Wg.copy(); dnm = np.zeros(K, bool); dnm[np.asarray(dn, int)] = True
    ornm = np.zeros(K, bool); ornm[ORN_IDX] = True; nsw = 0
    for _ in range(200000):
        sh = smell_shortcut_share(W)
        if (add and sh >= target) or ((not add) and sh <= target): break
        ri, rj = np.nonzero(W[:, ORN_IDX] != 0); rj = ORN_IDX[rj]                    # ORN edges (row = target)
        sel = (~dnm[ri]) if add else dnm[ri]
        if not sel.any(): break
        k = nrng.integers(0, sel.sum()); i1, j1 = ri[sel][k], rj[sel][k]
        yi, yj = np.nonzero(W != 0); ok = (~sensor[yj]) & (dnm[yi] if add else (~dnm[yi] & ~sensor[yi]))
        if not ok.any(): break
        k2 = nrng.integers(0, ok.sum()); i2, j2 = yi[ok][k2], yj[ok][k2]
        if W[i2, j1] != 0 or W[i1, j2] != 0 or i1 == i2: continue
        W[i2, j1], W[i1, j1] = W[i1, j1], 0.0
        W[i1, j2], W[i2, j2] = W[i2, j2], 0.0
        nsw += 1
    return W, nsw

def sham_rewire(Wg, nrng, nswaps):
    """Degree-preserving target swaps matched in number to the full transplant, but restricted to edge pairs whose
    sources are not olfactory receptors and whose targets are not descending groups, so the olfactory shortcut share
    is unchanged. Controls for the amount of rewiring the transplant does."""
    W = Wg.copy(); dnm = np.zeros(K, bool); dnm[np.asarray(dn, int)] = True
    ornm = np.zeros(K, bool); ornm[ORN_IDX] = True; done = 0
    yi, yj = np.nonzero(W != 0); ok = (~ornm[yj]) & (~sensor[yj]) & (~dnm[yi]); cand = np.where(ok)[0]
    for _ in range(nswaps * 50):
        if done >= nswaps: break
        a, b = nrng.integers(0, len(cand), 2); i1, j1, i2, j2 = yi[cand[a]], yj[cand[a]], yi[cand[b]], yj[cand[b]]
        if i1 == i2 or j1 == j2 or W[i2, j1] != 0 or W[i1, j2] != 0: continue
        W[i2, j1], W[i1, j1] = W[i1, j1], 0.0
        W[i1, j2], W[i2, j2] = W[i2, j2], 0.0
        yi[cand[a]], yi[cand[b]] = i2, i1; done += 1
    return W, done

AS_NSW = {}

def boundary_sham(Wg, nrng, nrot, cands=64):
    """Rotations ORN->X, Y->DN, Z->W  =>  ORN->W, Y->X, Z->DN (see header). Returns W, rotations done, |w| ratios."""
    W = Wg.copy(); dnm = np.zeros(K, bool); dnm[np.asarray(dn, int)] = True
    done = 0; ratios = []
    for _ in range(nrot * 200):
        if done >= nrot: break
        ri, rj = np.nonzero(W[:, ORN_IDX] != 0); rj = ORN_IDX[rj]; sel = ~dnm[ri]
        k = nrng.integers(0, sel.sum()); i1, j1 = ri[sel][k], rj[sel][k]                     # ORN -> X
        yi, yj = np.nonzero(W != 0); ok = (~sensor[yj]) & dnm[yi]
        k2 = nrng.integers(0, ok.sum()); i2, j2 = yi[ok][k2], yj[ok][k2]                      # Y -> DN
        if W[i1, j2] != 0 or i1 == i2: continue                                               # Y->X must be new
        zi, zj = np.nonzero(W != 0); okz = (~sensor[zj]) & (~dnm[zi]) & (~sensor[zi])
        idx = np.where(okz)[0]; pick = nrng.choice(idx, min(cands, len(idx)), replace=False)
        best = None
        for q in pick:
            i3, j3 = zi[q], zj[q]
            if i3 in (i1, i2) or j3 in (j1, j2): continue
            if W[i3, j1] != 0 or W[i2, j3] != 0: continue                                     # ORN->W, Z->DN must be new
            d = abs(abs(W[i3, j3]) - abs(W[i1, j1]))
            if best is None or d < best[0]: best = (d, i3, j3)
        if best is None: continue
        _, i3, j3 = best
        w1, w2, w3 = W[i1, j1], W[i2, j2], W[i3, j3]
        W[i1, j1] = 0.0; W[i2, j2] = 0.0; W[i3, j3] = 0.0
        W[i3, j1] = w1; W[i1, j2] = w2; W[i2, j3] = w3
        ratios.append(float(abs(w3) / (abs(w1) + 1e-12))); done += 1
    return W, done, ratios

def make_condition(cond, nrng):
    if cond.startswith("BS"):                                       # BS1, BS3, BS5, BS10: matched to this seed's AS<pct>
        key = "AS" + cond[2:]
        assert key in AS_NSW, f"{cond} needs {key} earlier in --conds"
        Wn, done, ratios = boundary_sham(W0, nrng, AS_NSW[key])
        same = bool(np.array_equal((W0 != 0).sum(1), (Wn != 0).sum(1)) and np.array_equal((W0 != 0).sum(0), (Wn != 0).sum(0)))
        print(f"{cond}: {done}/{AS_NSW[key]} boundary rotations, smell->DN share {smell_shortcut_share(W0):.4f} -> {smell_shortcut_share(Wn):.4f}; "
              f"nnz {int((W0 != 0).sum())}->{int((Wn != 0).sum())}; in/out-degree kept {same}; "
              f"|w(Z->DN)|/|w(ORN->X)| median {np.median(ratios) if ratios else float('nan'):.2f}", flush=True)
        assert done == AS_NSW[key] and same and smell_shortcut_share(Wn) <= smell_shortcut_share(W0) + 1e-9, f"{cond} construction failed"
        return Wn, Wpk0.copy()
    if cond.startswith("AS") and cond != "AS":                      # AS1, AS3, AS5, AS10 -> target share in percent
        tgt = float(cond[2:]) / 100.0
        Wn, nsw = transplant_shortcuts(W0, nrng, tgt, add=True)
        AS_NSW[cond] = nsw
        print(f"{cond}: target {tgt:.3f} -> smell->DN share {smell_shortcut_share(Wn):.4f} after {nsw} swaps; nnz {int((Wn != 0).sum())}", flush=True)
        return Wn, Wpk0.copy()
    if cond == "SHAM":
        _, nsw = transplant_shortcuts(W0, np.random.default_rng(777), SHORTCUT_TARGET, add=True)   # swap budget of the full transplant
        Wn, done = sham_rewire(W0, nrng, nsw)
        print(f"SHAM: {done}/{nsw} rewiring swaps, smell->DN share {smell_shortcut_share(W0):.4f} -> {smell_shortcut_share(Wn):.4f}; nnz {int((Wn != 0).sum())}; "
              f"in/out-degree kept {bool(np.array_equal((W0 != 0).sum(1), (Wn != 0).sum(1)) and np.array_equal((W0 != 0).sum(0), (Wn != 0).sum(0)))}", flush=True)
        return Wn, Wpk0.copy()
    if cond in ("AS", "N2R"):
        if cond == "AS":
            Wn, _ = transplant_shortcuts(W0, nrng, SHORTCUT_TARGET, add=True); Pn = Wpk0.copy()
        else:
            Wn, Pn = null_degswap(W0, Wpk0, nrng); Wn, _ = transplant_shortcuts(Wn, nrng, 0.0, add=False)
        print(f"{cond}: smell->DN shortcut share {smell_shortcut_share(W0):.4f} (connectome) -> {smell_shortcut_share(Wn):.4f}; "
              f"nnz {int((W0 != 0).sum())}->{int((Wn != 0).sum())}; in/out-degree kept "
              f"{bool(np.array_equal((W0 != 0).sum(1), (Wn != 0).sum(1)) and np.array_equal((W0 != 0).sum(0), (Wn != 0).sum(0))) if cond == 'AS' else 'n/a'}", flush=True)
        return Wn, Pn
    if cond in ("N4", "N5"):
...
def mutate(g, n):
    ...
    W = mx.put_along_axis(W, i1, mx.take_along_axis(W, i1, 1) * mx.exp(mx.random.normal((n, M1)) * 0.25), 1); mx.eval(W)  # MEM
    i2 = mx.random.randint(0, K * K, (n, M2)); cur = mx.take_along_axis(W, i2, 1)
    rowok = mx.take(C((~sensor).astype(np.float32)), i2 // K)
    W = mx.put_along_axis(W, i2, mx.where((cur == 0) & (rowok > 0), mx.take(C(dsign), i2 % K) * mx.abs(mx.random.normal((n, M2))) * 0.05, cur), 1); mx.eval(W)  # MEM
    i3 = mx.random.randint(0, K * K, (n, M3)); cur = mx.take_along_axis(W, i3, 1)
    W = mx.put_along_axis(W, i3, mx.where(mx.random.uniform(shape=(n, M3)) < 0.5, 0.0, cur), 1); mx.eval(W)  # MEM
    W = mx.where(mx.abs(W) < 1e-3, 0.0, W); W = W.reshape(n, K, K).astype(DT); mx.eval(W)  # MEM
    def pert(x, sig, frac, lo, hi):
        return mx.clip(x + (mx.random.uniform(shape=x.shape) < frac) * mx.random.normal(x.shape) * sig, lo, hi)
    out = dict(g)
    out.update(W=W, size=pert(g["size"], 0.15, 0.05, -2, 2), alpha=pert(g["alpha"], 0.3, 0.05, -4, 4),
...
for gen in range(args.gens):
    ...
    if gen % 250 == 0 or gen == args.gens - 1:                    # true elites: top 16 of this generation, before replacement
        np.savez_compressed(f"{OUT}/elite_g{gen:06d}.npz", pred_v=pred_v,
                            **{f"{isl}_{k}": npf(mx.take(v, C(sels[i][2][:16]), axis=0)) for i, isl in enumerate(LAB) for k, v in pop.items() if k != "Wpk_idx"})
    if gen == args.gens - 1: final_orders = [o for _, _, o in sels]; final_pop = pop
    ...
    ga = take(pop, idx); mx.eval(ga); pop = None  # MEM
    gm = mutate(ga, len(idx)); mx.eval(gm)  # MEM
...
def fresh_eval(gp_pop, orders, seeds=(10001, 10002, 10003, 10004), top=16, reps=8):
    """Final elites on fresh world batches: intact and anosmic, one line per batch in fresh.jsonl."""
    ids = np.concatenate([o[:top] for o in orders]); per = top * reps; blk = per * NI
    sel = np.tile(np.repeat(ids, reps), 2); n = len(sel)
    gp = with_shared(take(gp_pop, sel))
    m = np.ones((n, 3), np.float32); m[blk:2 * blk, 0] = 0                                  # second block: olfaction blocked
    swp = np.zeros(n, np.float32); pt = np.full(n, args.probe_ptox, np.float32)
    with open(f"{OUT}/fresh.jsonl", "a") as fl:
        for sd in seeds:
            mx.random.seed(sd); r = evaluate(gp, n, pred_v, C(m), C(swp), ptox=pt)
            out = dict(gen=args.gens - 1, world_seed=sd)
            for ci, cn in enumerate(("normal", "anosmic")):
                for ii, isl in enumerate(LAB):
                    s0 = ci * blk + ii * per; out[f"{isl}.{cn}"] = dict(fit=float(npf(r["fit"][s0:s0 + per]).mean()))
            fl.write(json.dumps(out) + "\n")
    mx.random.seed(int(time.time() * 1000) % 1000003)

if 'final_orders' in globals():
    fresh_eval(final_pop, final_orders)
