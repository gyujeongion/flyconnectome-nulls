"""evo_int.py: is the recurrent interior doing work? Re-evaluates stored populations with the interior wiring zeroed or scrambled.
...
if args.pathstats:
    # path statistics of the constructed conditions, before any mutation (Table 1 of the paper)
    from collections import deque
    dnm = np.zeros(K, bool); dnm[np.asarray(dn, int)] = True
    SRC = {"smell": np.array(sorted(set(int(i) for i in B["in_orn"]))),
           "vision": np.array(sorted(set(int(i) for i in B["in_loomL"]) | set(int(i) for i in B["in_loomR"]))),
           "taste": np.array(sorted(set(int(i) for i in B["in_sugar"]) | set(int(i) for i in B["in_bitter"])))}
    out = open(f"{OUT}/pathstats.jsonl", "a")
    for ci, isl in enumerate(LAB):
        Wc = np.array(calib[isl][0], dtype=np.float64)   # calibrated weights actually used in the runs
        rec = {"cond": isl, "seed": args.seed}
        for nm, src in SRC.items():
            direct = float(np.abs(Wc[np.ix_(np.where(dnm)[0], src)]).sum() / (np.abs(Wc[:, src]).sum() + 1e-12))
            Aadj = np.abs(Wc) > 0; dist = np.full(K, 99); q = deque()
            for s_ in src: dist[s_] = 0; q.append(s_)
            while q:
                u = q.popleft()
                for v in np.nonzero(Aadj[:, u])[0]:
                    if dist[v] == 99: dist[v] = dist[u] + 1; q.append(v)
            rec[nm] = dict(direct=round(direct, 5), hops=int(np.median([dist[d_] for d_ in np.where(dnm)[0]])))
        out.write(json.dumps(rec) + "\n"); print("PATHSTATS", json.dumps(rec), flush=True)
    raise SystemExit
...
if args.assay:
    ...
    n0 = nel * NI
    nrng_assay = np.random.default_rng(1000 + args.seed)
    ...
    R_ = 12
    if args.assay_set == "interior":
        dnm = np.zeros(K, bool); dnm[np.asarray(dn, int)] = True
        IMASK = (~sensor)[None, :] & (~dnm)[:, None]           # interior = source not sensory, target not descending
        W_all = np.array(genome["W"], dtype=np.float32)
        for tag in ("intact", "interior_zeroed", "interior_scrambled"):
            Wm = W_all.copy()
            if tag == "interior_zeroed":
                Wm[:, IMASK] = 0.0
            elif tag == "interior_scrambled":
                for a_ in range(Wm.shape[0]):
                    v = Wm[a_][IMASK]; Wm[a_][IMASK] = nrng_assay.permutation(v)
            genome["W"] = C(Wm)
            run_block(tag, 6, args.ptox0, 0, False)
        raise SystemExit
