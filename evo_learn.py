...
FLIP_CANDIDATES = (0.0, 0.002, 0.005, 0.01)   # odor-valence flip rates the ecology search scores
AS_SWAPS = {}                                 # dose -> swap count used, filled as conditions are built
...
def make_condition(cond, nrng):
    if cond.startswith("AS") and cond != "AS":                      # AS1, AS3, AS5, AS10 -> target share in percent
        ...
        AS_SWAPS[cond[2:]] = nsw                                    # SHAM<pct> matches this exact count
        print(f"{cond}: target {tgt:.3f} -> smell->DN share {smell_shortcut_share(Wn):.4f} after {nsw} swaps; nnz {int((Wn != 0).sum())}", flush=True)
        return Wn, Wpk0.copy()
    if cond.startswith("SHAM"):
        if cond == "SHAM":
            # swap budget of the full transplant, computed on its own stream (as in PROTOCOL_DOSE)
            _, nsw = transplant_shortcuts(W0, np.random.default_rng(777), SHORTCUT_TARGET, add=True)
        else:
            # SHAM<pct> takes the swap count AS<pct> actually used in THIS run, so the pair is
            # matched exactly rather than approximately. AS<pct> must therefore be listed first.
            key = cond[4:]
            assert key in AS_SWAPS, f"{cond} needs AS{key} earlier in --conds so the swap count can be matched"
            nsw = AS_SWAPS[key]
        Wn, done = sham_rewire(W0, nrng, nsw)
        print(f"{cond}: {done}/{nsw} rewiring swaps, smell->DN share {smell_shortcut_share(W0):.4f} -> {smell_shortcut_share(Wn):.4f}; nnz {int((Wn != 0).sum())}; "
...
if args.ecosearch:
    ...
    for it in range(args.ecosearch):
        ...
        for fr in FLIP_CANDIDATES:
            nonlearn = max(res[f"noEat@{fr}"][0], res[f"reflex@{fr}"][0], res[f"sickStop@{fr}"][0])
            learn = max(res[f"twoMem@{fr}"][0], res[f"localSearch@{fr}"][0], res[f"trapline@{fr}"][0])
            score = dict(flip_rate=fr, learn_margin=round(learn - nonlearn, 3), learn_ratio=round(learn / max(nonlearn, 0.05), 3),
                         learner_surv=max(res[f"twoMem@{fr}"][1], res[f"localSearch@{fr}"][1], res[f"trapline@{fr}"][1]),
                         reflex_surv=res[f"reflex@{fr}"][1],
                         twomem_ratio=round(res[f"twoMem@{fr}"][0] / max(nonlearn, 0.05), 3),
                         local_ratio=round(res[f"localSearch@{fr}"][0] / max(nonlearn, 0.05), 3),
                         trap_ratio=round(res[f"trapline@{fr}"][0] / max(nonlearn, 0.05), 3), nonlearn_best=round(nonlearn, 3),
                         oracle_ratio=round(res[f"oracle@{fr}"][0] / max(nonlearn, 0.05), 3))
            out.write(json.dumps(dict(cfg=dict(cfg, flip_rate=fr), score=score, res=res)) + "\n"); out.flush()
            print(f"ECO {it} fr={fr} {json.dumps(cfg)} -> {json.dumps(score)}", flush=True)
    raise SystemExit

if args.wlearn:
    # can an agent with a hand-ALIGNED MBON->eat readout learn odor->toxin in this world? (substrate + world learnability)
    n = 512; i0 = 0
    # calib entries carry the homeostatic bias as a fourth element; this diagnostic predates
    # that and unpacked three, so it raised ValueError the moment it was called.
    Wg, Wp, kg, _bias = calib["A"]
    CUR_BIAS = _bias
