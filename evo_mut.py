"""evo_mut.py: mutational-neighbourhood assay (PROTOCOL_MUT.md) + post-hoc assay runner = frozen evo_paper.py + --assay_set. Evolution code untouched.
...
def world_step(st, Wkm, g, nz, half, pred_v, mask, pol, cg):
    ...
    th = th + (E["wmax"] * mx.tanh(act[:, 1]) + 0.05 * nz[:, 0]) * alive
...
if args.assay:
    ...
    R_ = 12
    if args.assay_set == "mut":
        # PROTOCOL_MUT.md: one-step mutants of saved genotypes under the real GA operator, magnitude-scaled
        def mutate_scaled(g, n, s):
            W = g["W"].astype(mx.float32).reshape(n, K * K)
            M1, M2, M3 = 3000, 3, 60
            i1 = mx.random.randint(0, K * K, (n, M1))
            W = mx.put_along_axis(W, i1, mx.take_along_axis(W, i1, 1) * mx.exp(mx.random.normal((n, M1)) * 0.25 * s), 1)
            i2 = mx.random.randint(0, K * K, (n, M2)); cur = mx.take_along_axis(W, i2, 1)
            rowok = mx.take(C((~sensor).astype(np.float32)), i2 // K)
            W = mx.put_along_axis(W, i2, mx.where((cur == 0) & (rowok > 0), mx.take(C(dsign), i2 % K) * mx.abs(mx.random.normal((n, M2))) * 0.05 * s, cur), 1)
            i3 = mx.random.randint(0, K * K, (n, M3)); cur = mx.take_along_axis(W, i3, 1)
            W = mx.put_along_axis(W, i3, mx.where(mx.random.uniform(shape=(n, M3)) < 0.5, 0.0, cur), 1)
            W = mx.where(mx.abs(W) < 1e-3, 0.0, W)
            def pert(x, sig, frac, lo, hi):
                return mx.clip(x + (mx.random.uniform(shape=x.shape) < frac) * mx.random.normal(x.shape) * sig * s, lo, hi)
            out = dict(g)
            out.update(W=W.reshape(n, K, K).astype(DT), size=pert(g["size"], 0.15, 0.05, -2, 2), alpha=pert(g["alpha"], 0.3, 0.05, -4, 4),
                       bias=pert(g["bias"], 0.1, 0.05, -2, 2), eta=pert(g["eta"], 0.03, 0.2, -1, 1), gin=pert(g["gin"], 0.2, 0.3, 0, 8),
                       kcg=pert(g["kcg"], 0.1, 0.3, -2, 2), kcl=pert(g["kcl"], 0.1, 0.3, 0.5, 3.0),
                       R=pert(g["R"], 0.5, 0.2, -12, 12), RM=pert(g["RM"], 0.5, 0.2, -10, 10), Rb=pert(g["Rb"], 0.2, 0.3, -5, 5))
            return out
        SC, MM, RP, CH = (0.5, 1.0, 2.0), 32, 4, 512
        pv_ = max(args.pred_fixed, 0.0) * E["vmax"]; pt_ = args.ptox0
        def fit_of(g, ng):
            res = []
            for s0 in range(0, ng, CH):
                idx = np.arange(s0, min(ng, s0 + CH)); sel = np.tile(idx, RP); n = len(sel)
                gp = with_shared(take(g, sel))
                r = evaluate(gp, n, pv_, C(np.ones((n, 3), np.float32)), C(np.zeros(n, np.float32)), ptox=np.full(n, pt_, np.float32))
                res.append(npf(r["fit"]).reshape(RP, len(idx)).mean(0))
            return np.concatenate(res)
        mx.random.seed(9000 + args.seed)
        sel = np.repeat(np.arange(n0), MM)
        base = take(genome, sel)
        clone = fit_of(base, len(sel)).reshape(n0, MM)
        mut = np.stack([fit_of(mutate_scaled(base, len(sel), s_), len(sel)).reshape(n0, MM) for s_ in SC])
        cond = np.repeat(np.array(LAB), nel)
        np.savez_compressed(f"{args.assay}/mut_g{args.assay_gen:06d}.npz", clone=clone, mut=mut, scales=np.array(SC), cond=cond)
        print(f"MUT gen {args.assay_gen}: " + " ".join(f"{c} clone {clone[cond == c].mean():.3f} mut1 {mut[1][cond == c].mean():.3f}" for c in LAB), flush=True)
        raise SystemExit
    if args.assay_set == "eco":
        # paper-v2 cross-ecology common garden only (PROTOCOL_ECO.md secondary endpoint)
        for pv_ in (0.0, 0.2):
            for pt_ in (0.0, 1.0):
