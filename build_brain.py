...
rein_keep = [int(i) for i in np.argsort(-path_rein)[:48] if path_rein[i] > 0 and names[i] != "KCBULK"]
...
mandatory = sorted(set(sum(inp.values(), []) + mbon_i + dan_i + dn_named + rein_keep + al_pn + al_ln
...
rest = [i for i in np.argsort(-score) if i not in set(mandatory) and not names[i].startswith(("ORN_other", "GRN_other", "KCBULK"))]
...
out = dict(W0=W0.astype(np.float32), Wpk=Wpk, Wko=Wko, names=names[keep], pn=pn_k, ln=ln_k, nneur=nneur[keep],
...
np.savez_compressed("data/brain.npz", **out)
print(json.dumps({k_: int(len(v_)) for k_, v_ in out.items() if hasattr(v_, "__len__") and k_ != "W0"}))
