"""evo_swap.py: evo_fix.py + shortcut-transplant conditions AS / N2R (PROTOCOL_SWAP.md).
...
def transplant_shortcuts(Wg, nrng, target, add):
    ...
    ornm = np.zeros(K, bool); ornm[ORN_IDX] = True
...
def make_condition(cond, nrng):
    if cond in ("AS", "N2R"):
        if cond == "AS":
            Wn = transplant_shortcuts(W0, nrng, SHORTCUT_TARGET, add=True); Pn = Wpk0.copy()
        else:
            Wn, Pn = null_degswap(W0, Wpk0, nrng); Wn = transplant_shortcuts(Wn, nrng, 0.0, add=False)
