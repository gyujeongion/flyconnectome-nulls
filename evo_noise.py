"""evo_noise.py: turning-noise robustness assay (defect 28) + post-hoc assay runner = frozen evo_paper.py + --assay_set. Evolution code untouched.
...
def world_step(st, Wkm, g, nz, half, pred_v, mask, pol, cg):
    ...
    th = th + (E["wmax"] * mx.tanh(act[:, 1]) + 0.05 * (nz[:, 0] - 0.5 * args.zm_noise)) * alive   # zm_noise=1: zero-mean turning noise
...
if args.assay:
    ...
    R_ = 12
    if args.assay_set == "noise":
        run_block(f"noise_zm{args.zm_noise}", R_, args.ptox0, 0, False)
        raise SystemExit
